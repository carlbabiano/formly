...
import {
  ...
  BadgeAlert, Clock, Eye, Upload, MoveVertical, Grid, CheckCircle2, XCircle, ToggleLeft, Trash2, Link2, Link, Save,
} from "lucide-react"
...
const form = location.state?.form; // Retrieve the form data from state
    ...
    const fetchSurvey = async () => {
      try {
        ...
        if (data.success) {
          ...
          // Only update questions if they are not already set
          if (!questions.length) {
          ...
          setCreatorEmail(data.survey.creator_email || localStorage.getItem("email")); // Use backend email or fallback to local storage
        } else {
        }
      } catch (error) {
      }
  ...
  const saveRecentForm = async (surveyData) => {
    ...
    try {
      const response = await fetch("http://localhost/formlydb/formly/src/backend/saveRecentForm.php", {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    const validateToken = async () => {
      ...
      try {
        const response = await fetch("http://localhost/formlydb/formly/src/backend/validateToken.php", {
          method: "POST",
